...
df['Maand'] = df['STD'].dt.to_period('M')  # Gebruik Period voor maandelijkse data
...
for luchthaven in luchthavens:
    ...
    data_per_luchthaven = data_per_luchthaven.asfreq('M')  # Maandelijkse frequentie

    # Controleer of er voldoende data is om het model te trainen
    if len(data_per_luchthaven) < 12:
        print(f"Te weinig data voor {luchthaven}, overslaan.")
        continue
    ...
    # Voorspellingen genereren voor de gehele dataset
    ...
    fig.add_trace(go.Scatter(
        x=data_per_luchthaven.index.to_timestamp(), 
...
for i, luchthaven in enumerate(luchthavens):
    ...
    visible_trace_indices[i * 2: i * 2 + 2] = [True] * 2  # Toon alleen de sporen voor de geselecteerde luchthaven
    dropdown_buttons.append(
        dict(
            label=str(luchthaven),
            method="update",
